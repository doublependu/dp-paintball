...
async function holdAndRead(keys, ms) {
  for (const k of keys) await page.keyboard.down(k);
  await page.waitForTimeout(ms);
...
/** Polls for `ms`, returning the highest y seen — used to catch a jump apex. */
async function peakHeight(ms) {
  let peak = -Infinity;
...
async function climbFrom(x, y, z, yaw, ms) {
...
const walkEnd = await holdAndRead(['w'], 1200);
...
// --- Sprint ---------------------------------------------------------------
await reset(0, 1, 20);
const sprintEnd = await holdAndRead(['w', 'Shift'], 1200);
check(
...
const apex = await peakHeight(900);
...
const steps = await climbFrom(3, 1, -24, Math.PI, 2600);
check(
...
const tallSteps = await climbFrom(10, 1, -24, Math.PI, 2600);
...
const wall = await climbFrom(-11.5, 1, 14, Math.PI / 2, 2600);
...
// unambiguous.
const ramp45 = await climbFrom(-24, 1, -20, Math.PI, 4500);
check('climbs the 45 degree ramp', ramp45.gain > 2.5, `rose ${ramp45.gain.toFixed(2)}m`);

const ramp60 = await climbFrom(-16, 1, -20, Math.PI, 4500);
check(
